...
private openAddFriendPopup(): void {
  ...
  otpButton.addEventListener("click", async () => {
    ...
    const verifyRes = await this.profileService.verify2FA(code);
  ...
  const init2FAStatus = async () => {
    const res = await this.profileService.get2FAStatus();
  ...
  toggleSwitch.addEventListener("click", async () => {
    ...
    if (isEnabled) {
      const res = await this.profileService.disable2FA();
      if (res.success) {
        toggleSwitch.classList.remove("enabled", "bg-[#77AB55]");
        toggleCircle.style.transform = "translate(-0%, -50%)";
        toggleCircle.style.left = "4px";
      ...
      this.profileService.enable2FA().then(res => {
